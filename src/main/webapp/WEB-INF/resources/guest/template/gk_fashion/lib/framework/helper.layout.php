...
class GKTemplateLayout {
	...
	public function generateLayoutWidths() {
		...
		$inset_width = $this->API->get('layout_inset_width', 200); // get the inset width
    ...
    public function generateLayout($offset) {
    	...
    	// inset container
    	$this->API->addCSSRule('#gkInset { width: '.($inset_width).'px; padding: '.$grid_spaces.'px; padding-left: 0; }');
    	// space for the inset container
    	if($this->API->modules('inset')) {
    		$this->API->addCSSRule('#gkPage { margin-left: '.($inset_width).'px; padding-left: '.$grid_spaces.'px; }');
    	}
    	// calculate the minimal width for the content
    	$min_content_width = ($body_padding * 2) + ($grid_base * $content_width);
    	//
    	...
    	$this->API->addCSSRule('body #gkPageTop { padding-left: '.$body_padding.'px; padding-right: '.$body_padding.'px; margin-left: '.(-1 * $body_padding).'px!important; }');
